...
export function SudokuGrid({ grid, onChange, readOnly = false, initial, highlights }: SudokuGridProps) {
  return (
    // Fix: use a stable width so multiple grids placed side-by-side don't
    // overlap due to auto-sizing. 9 cells × 40px each ≈ 360px.
    <div className="grid grid-cols-9 gap-0 border-2 border-gray-800 w-[360px] mx-auto">
      {grid.map((row, r) =>
        row.map((cell, c) => {
            ...
            >
              <Input
                ...
                onChange={(e) => {
                  ...
                  else if (/[1-9]/.test(val)) onChange(r, c, Number.parseInt(val))
                }}
              />
            </div>
          )
