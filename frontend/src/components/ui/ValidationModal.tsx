...
export function ValidationModal({
  ...
  if (mode === "competitive" && submissions.length >= 2) {
    ...
    const nameA =
      playerLabels?.[submissions[0].clientId ?? ""] ?? submissions[0].name ?? submissions[0].clientId ?? "Player 1"
    const nameB =
      playerLabels?.[submissions[1].clientId ?? ""] ?? submissions[1].name ?? submissions[1].clientId ?? "Player 2"
